...
def parseHTML(html):
    ...
    for info in movie4:
        ...
        movie_list.append([year,score,peo])
...
def writeCSV(file_name, data_list):
    with codecs.open(file_name, 'w+','utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["time","score","people"])#write要有标题就这样
...
try:
    for i in range(99):
        num = str(20*(i))#这里转str
        url = 'https://movie.douban.com/tag/战争?start='+num+'&type=T'    #这个url的构造规律性太强了！
        html = getHTML(url)
        data_list = data_list + parseHTML(html) # list拼接
        print(i,data_list)
    writeCSV('warscore.csv',data_list)
